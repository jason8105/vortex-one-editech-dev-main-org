...
public class BPackage implements Parcelable {
    ...
    public BPackage(PackageParser.Package aPackage) {
        top.niunaijun.blackbox.utils.Slog.d("BPackage", "Parsing BPackage for: " + aPackage.packageName);
        ...
        try {
            this.baseCodePath = Reflector.with(aPackage).field("baseCodePath").get();
        } catch (Throwable e) {
            // e.printStackTrace();
            // top.niunaijun.blackbox.utils.Slog.d("BPackage",
            // "baseCodePath reflection failed for " + aPackage.packageName + ": " +
            // e.getMessage());
            if (this.applicationInfo != null) {
                this.baseCodePath = this.applicationInfo.sourceDir;
                // top.niunaijun.blackbox.utils.Slog.d("BPackage", "Using fallback baseCodePath:
                // " + this.baseCodePath);
            } else {
                top.niunaijun.blackbox.utils.Slog.d("BPackage",
                        "CRITICAL: applicationInfo is null, cannot fallback baseCodePath!");
            }
        }
        ...
        // this.mSignatures = aPackage.mSignatures;
        // this.mSigningDetails = aPackage.mSigningDetails;
        try {
            // SAFE REFLECTION + WRAPPING for mSigningDetails
            Object frameworkDetails = Reflector.with(aPackage).field("mSigningDetails").get();
            if (frameworkDetails != null) {
                this.mSigningDetails = new SigningDetails((PackageParser.SigningDetails) frameworkDetails);
            } else {
                // top.niunaijun.blackbox.utils.Slog.d("BPackage", "mSigningDetails is null for
                // " + aPackage.packageName);
            }
        } catch (Throwable e) {
            // top.niunaijun.blackbox.utils.Slog.d("BPackage",
            // "mSigningDetails reflection failed for " + aPackage.packageName + ": " +
            // e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            if (this.mSigningDetails != null) {
                ...
                this.mSignatures = Reflector.with(aPackage).field("mSignatures").get();
            }
        } catch (Throwable e) {
            // top.niunaijun.blackbox.utils.Slog.d("BPackage",
            // "mSignatures reflection failed for " + aPackage.packageName + ": " +
            // e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            // Android 9/Pie field
            // public Signature[] pastSigningCertificates;
            this.pastSigningCertificates = Reflector.with(aPackage).field("pastSigningCertificates").get();
        } catch (Throwable e) {
            // ignore
        }
    }
    ...
    public static final class SigningDetails implements Parcelable {
        ...
        public SigningDetails(PackageParser.SigningDetails signingDetails) {
            Signature[] pastSigningCertificates = null;
            try {
                pastSigningCertificates = Reflector.with(signingDetails).field("pastSigningCertificates").get();
            } catch (Throwable e) {
                // ignore
            }
            if (pastSigningCertificates == null) {
                ...
                this.signatures = pastSigningCertificates;
            }
        }
    ...
    public static class IntentInfo implements Parcelable {
        ...
        public IntentInfo(PackageParser.IntentInfo intentInfo) {
            ...
            this.nonLocalizedLabel = intentInfo.nonLocalizedLabel == null ? null
                    : intentInfo.nonLocalizedLabel.toString();
