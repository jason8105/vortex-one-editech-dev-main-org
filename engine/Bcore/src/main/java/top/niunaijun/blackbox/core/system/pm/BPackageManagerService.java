        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                try {
                    ...
                    // Fix for Fire OS 6 / Android 7.1.2: Use reflection to set final field 'applicationInfo'
                    top.niunaijun.blackbox.utils.Reflector.with(aPackage)
                            .field("applicationInfo")
                            .set(appInfo);
                } catch (Exception e) {
                    // Packet not found on host, or reflection failed. 
                    // This is expected for our "Manual Safe Install" strategy where we use FLAG_SYSTEM for a non-system app.
                    // effectively falling back to using the parsed package info.
                    // Slog.d(TAG, "FLAG_SYSTEM used but package not on host or reflection failed. Using parsed info.");
                }
            }
