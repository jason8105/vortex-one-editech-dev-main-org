        ...
        if (!mBeing.getAndSet(true)) {
            try {
                if (BuildCompat.isPie()) {
                    Integer transactionCode = BRActivityThreadH.get().EXECUTE_TRANSACTION();
                    if ((transactionCode != null && msg.what == transactionCode) || msg.what == 159) {
                ...
                Integer launchActivityCode = BRActivityThreadH.get().LAUNCH_ACTIVITY();
                if ((launchActivityCode != null && msg.what == launchActivityCode) || msg.what == 100) {
                    if (handleLaunchActivity(msg.obj)) {
                        getH().sendMessageAtFrontOfQueue(Message.obtain(msg));
                        return true;
                    }
    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                BlackBoxCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName,
                        stubRecord.mUserId);
                ...
                ProxyActivityRecord.saveStub(intent, launchIntentForPackage, stubRecord.mActivityInfo,
                ...
                if (BuildCompat.isPie()) {
                    ...
                    ActivityThreadActivityClientRecordContext clientRecordContext = BRActivityThreadActivityClientRecord
                            .get(r);
            ...
            // bind
            // bind
            // Slog.d(TAG, "handleLaunchActivity: isInit=" +
            ...
            if (!BActivityThread.currentActivityThread().isInit()) {
                ...
                // Slog.d(TAG, "handleLaunchActivity: bindApplication completed, isInit now="
                // + BActivityThread.currentActivityThread().isInit());
                // Only return false if bindApplication failed (isInit still false)
                // This handles devices like Hisense where isInit stays false
                if (!BActivityThread.currentActivityThread().isInit()) {
                    // Slog.w(TAG, "bindApplication completed but isInit still false, using
                    // fallback");
                    return false; // Fallback to ProxyActivity.onCreate()
                }
                return true;
            }

            int taskId = BRIActivityManager.get(BRActivityManagerNative.get().getDefault()).getTaskForActivity(token,
                    false);
